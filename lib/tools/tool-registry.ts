...
const frameLocations: ToolImplementation = async (args, context) => {
  ...
  if (shouldCreateMarkers) {
    ...
    let adjustedRange = cameraProps.range;
    switch (zoomLevel) {
      case 'close':
        adjustedRange = cameraProps.range * 0.7; // Tighter zoom
        break;
      case 'far':
        adjustedRange = cameraProps.range * 1.5; // Wider zoom
        break;
      case 'medium':
      default:
        // Use default calculated range with buffer
        break;
    }
    ...
    useMapStore.getState().setCameraTarget({
      ...
      range: adjustedRange + 1000, // Add buffer to the adjusted range
      heading: cameraProps.heading,
      tilt: cameraProps.tilt,
      roll: 0,
    });

    return `Framed ${locationsWithLabels.length} locations on the map with a '${zoomLevel || 'medium'}' zoom level.`;
  }
};
