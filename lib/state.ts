...
const LOCAL_STORAGE_KEY = 'gemini-maps-chat-history';
...
export const useLogStore = create<{
  ...
  // New: Save turns to localStorage
  saveTurns: () => {
    try {
      const serializedState = JSON.stringify(get().turns);
  ...
  // New: Load turns from localStorage
  loadTurns: () => {
    try {
      ...
      const loadedTurns: ConversationTurn[] = JSON.parse(serializedState);
      // Re-hydrate Date objects
      const rehydratedTurns = loadedTurns.map(turn => ({
        ...turn,
        timestamp: new Date(turn.timestamp),
      }));
      set({ turns: rehydratedTurns });
      useLogStore.getState().addTurn({
        role: 'system',
        text: `Loaded ${rehydratedTurns.length} turns from previous session.`,
        isFinal: true,
      });
    } catch (e) {
      ...
      useLogStore.getState().addTurn({
        role: 'system',
        text: `Failed to load previous session history. Starting fresh.`,
        isFinal: true,
      });
      set({ turns: [] }); // Clear turns if loading fails
    }
  },
}));

// Subscribe to changes in turns to automatically save
useLogStore.subscribe(
  (state) => state.turns,
  (turns) => {
    if (turns.length > 0) { // Only save if there's actual content
      useLogStore.getState().saveTurns();
    }
  },
  {
    // Fire immediately after store creation to load initial state
